...
// TODO: Agent identity icon system not yet extracted from ui/ module.
// Stubbed here for basic functionality.
export type AgentFamily = 'purple' | 'blue' | 'green' | 'orange' | 'red' | string;

function createAgentFaceElement(opts: {
  family: AgentFamily;
  size: number;
  title: string;
  ...
  if (opts.wrapperClassName) span.className = opts.wrapperClassName;
  span.textContent = '\u{1F916}'; // Robot face fallback
  span.style.fontSize = `${opts.size}px`;
  return span;
}

function getAgentFacePalette(_family: AgentFamily): { accent: string } {
  return { accent: '#2563eb' };
}

function resolveAgentFamily(_identity: { actor?: string; name?: string; avatar?: string }): AgentFamily {
  return 'purple';
}
  ...
  content.innerHTML = entries.map(entry => {
    ...
    switch (entry.type) {
      case 'thinking':
        ...
        break;
      case 'tool_call':
        html += `<div class="agent-thinking-tool-name">Tool: ${escapeHtml(entry.toolName || '')}</div>`;
        if (entry.content) {
          html += `<div>${escapeHtml(entry.content)}</div>`;
        }
        break;
      case 'tool_result':
        html += `<div class="agent-thinking-tool-name">Done: ${escapeHtml(entry.toolName || '')}</div>`;
        break;
      case 'text':
        html += `<div>${escapeHtml(entry.content || '')}</div>`;
        break;
      case 'error':
        html += `<div>Error: ${escapeHtml(entry.content || 'An error occurred')}</div>`;
        break;
    }
