// ---------------------------------------------------------------------------
// @proof-clone/agent-bridge — Pure HTTP client for the Proof bridge API.
// Zero server dependencies. Uses fetch (native or injected).
// ---------------------------------------------------------------------------

// ---- Bridge route type definitions (inlined from bridge-routes) -----------
...
// ---- Client configuration & input/output interfaces ----------------------
...
export function createAgentBridgeClient(config: AgentBridgeClientConfig) {
  return {
    ...
    addComment<T = unknown>(
      slug: string,
      input: AgentBridgeCommentInput,
    ...
    addSuggestion<T = unknown>(
      slug: string,
      input: AgentBridgeSuggestionInput,
      options: AgentBridgeRequestOptions = {},
    ): Promise<T> {
    ...
    setPresence<T = unknown>(
      slug: string,
      input: AgentBridgePresenceInput,
      options: AgentBridgeRequestOptions = {},
    ): Promise<T> {
